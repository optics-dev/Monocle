...
final class NamedTupleAppliedFocusTest extends munit.FunSuite {
  ...
  test("Applied focus returning an Optional in nested named tuples") {
    ...
    // the macro cannot find a matching overload otherwise - note that this also happens in a normal .focus incovation on case classes when a type is described as Some instead of Option
