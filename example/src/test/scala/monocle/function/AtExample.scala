...
class AtExample extends MonocleSuite {
  test("at creates a Lens from a Map, SortedMap to an optional value") {
    ...
    assertEquals((Map("One" -> 1, "Two" -> 2) applyLens at("One") set Some(-1)), Map("One" -> -1, "Two" -> 2))

    // can delete a value
    assertEquals((Map("One" -> 1, "Two" -> 2) applyLens at("Two") set None), Map("One" -> 1))

    // add a new value
    assertEquals(
      (Map("One" -> 1, "Two" -> 2) applyLens at("Three") set Some(3)),
      Map(
        "One"   -> 1,
        "Two"   -> 2,
        "Three" -> 3
  ...
  test("at creates a Lens from a Set to an optional element of the Set") {
    ...
    assertEquals((Set(1, 2, 3) applyLens at(4) set true), Set(1, 2, 3, 4))
    assertEquals((Set(1, 2, 3) applyLens at(2) set false), Set(1, 3))
  }
  ...
  test("at creates a Lens from Int to one of its bit") {
    ...
    assertEquals((32 applyLens at(0: IntBits) set true), 33)
  ...
  test("at creates a Lens from Char to one of its bit") {
    ...
    assertEquals(('x' applyLens at(0: CharBits) set true), 'y')
  }
