...
class AtExample extends Spec {
  ...
  "at creates a Lens from a Map, ==>>(IMap) to an optional value" in {
    ...
    (==>>("One" -> 2, "Two" -> 2) applyLens at("Two") get) ==== Maybe.just(2)

    (==>>("One" -> 1, "Two" -> 2) applyLens at("One") set Maybe.just(-1))  ==== ==>>("One" -> -1, "Two" -> 2)


    // can delete a value
    (Map("One" -> 1, "Two" -> 2) applyLens at("Two") set Maybe.empty) ==== Map("One" -> 1)
    (==>>("One" -> 1, "Two" -> 2) applyLens at("Two") set Maybe.empty) ==== ==>>("One" -> 1)

    // add a new value
    (Map("One" -> 1, "Two" -> 2) applyLens at("Three") set Maybe.just(3)) ==== Map("One" -> 1, "Two" -> 2, "Three" -> 3)
    (==>>("One" -> 1, "Two" -> 2) applyLens at("Three") set Maybe.just(3)) ==== ==>>("One" -> 1, "Two" -> 2, "Three" -> 3)
  }
