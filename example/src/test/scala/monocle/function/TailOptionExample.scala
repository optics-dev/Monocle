...
class TailOptionExample extends Spec {
  ...
  "tailOption creates an Optional from a List, IList, Vector or Stream to its tail" in {
    (List(1, 2, 3)    applyOptional tailOption getMaybe) ==== Maybe.just(List(2, 3))
    (List(1)          applyOptional tailOption getMaybe) ==== Maybe.just(Nil)
    ((Nil: List[Int]) applyOptional tailOption getMaybe) ==== Maybe.empty
