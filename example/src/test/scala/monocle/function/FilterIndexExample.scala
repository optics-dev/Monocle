...
class FilterIndexExample extends MonocleSuite {
  test("filterIndexes creates Traversal from a SortedMap, IMap to all values where the index matches the predicate") {
    (SortedMap("One" -> 1, "Two" -> 2) applyTraversal filterIndex { k: String => k.toLowerCase.contains("o") } getAll) shouldEqual List(
      1,
      2
    )
    (SortedMap("One" -> 1, "Two" -> 2) applyTraversal filterIndex { k: String => k.startsWith("T") } set 3) shouldEqual SortedMap(
      "One"          -> 1,
      "Two"          -> 3
    )
  }
