...
class LensExample extends Spec {
  ...
  "Lens get extract an A from an S" in {
    (john applyLens SimpleLensVerbose._name get)   shouldEqual "John"
    (john applyLens SimpleLensInferred._name get)  shouldEqual "John"
    (john applyLens MkLensMacro.name get)          shouldEqual "John"
    (john applyLens LenserMacro.name get)          shouldEqual "John"
    (john applyLens Person.name get)               shouldEqual "John"
  }

  "Lens set and modify update an A in a S" in {
    val changedJohn = Person("John", 45)

    (john applyLens SimpleLensVerbose._age set 45)  shouldEqual changedJohn
    (john applyLens SimpleLensInferred._age set 45) shouldEqual changedJohn
    (john applyLens MkLensMacro.age set 45)         shouldEqual changedJohn
    (john applyLens LenserMacro.age set 45)         shouldEqual changedJohn
    (john applyLens Person.age set 45)              shouldEqual changedJohn
  }
  ...
  "@Lenses takes an optional prefix string" in {
    (alpha applyLens Cat._age get)   shouldEqual 2
    (alpha applyLens Cat._age set 3) shouldEqual Cat(3)
  }
