...
class ValidatedExample extends MonocleSuite {
  test("success defines a Prism that can get or replace the underlying value of a Success instance") {
    assertEquals(mValidated.success.getOption(123.valid), Some(123))
    assertEquals(mValidated.success.getOption("abc".invalid), None)

    assertEquals(mValidated.success.replace(555)(123.valid), 555.valid)
    assertEquals(mValidated.success.replace(123)("abc".invalid), "abc".invalid)
  }
