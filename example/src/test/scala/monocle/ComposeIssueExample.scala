...
class ComposeIssueExample extends Spec {
  ...
  "compose same class" in {
    (aI2S compose aS2S).isInstanceOf[A[_, _]] ==== true
    (aS2S compose aS2S).isInstanceOf[A[_, _]] ==== true
    (bI2S compose bS2S).isInstanceOf[B[_, _]] ==== true
  }

  "compose different class" in {
    (aI2S compose bS2S).isInstanceOf[B[_, _]] ==== true
    (bI2S compose aS2S).isInstanceOf[B[_, _]] ==== true
  }
  ...
  "compose with parametric method" in {
    // explicit type
    (aI2S compose b2S[String]).isInstanceOf[B[_, _]] ==== true
    // non overloaded method
    (aI2S composeB b2S).isInstanceOf[B[_, _]] ==== true
