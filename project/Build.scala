...
object BuildSettings {
  ...
  val buildSettings = Defaults.defaultSettings ++ Seq(
    ...
    addCompilerPlugin("org.scalamacros" % "paradise" % "2.0.0-M3" cross CrossVersion.full)) ++ publishSettings
}
...
object ScalaLensBuild extends Build {
  ...
  lazy val root: Project = Project(
    ...
    settings = buildSettings ++ Seq(
      ...
      run <<= run in Compile in core) ++ sonatypeSettings) aggregate (core, examples)
  ...
  lazy val examples: Project = Project(
    ...
    settings = buildSettings ++ Seq(
      publishArtifact := false,
      libraryDependencies ++= Seq(scalaz, shapeless))) dependsOn (core)
}
