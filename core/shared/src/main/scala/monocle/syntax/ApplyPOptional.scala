...
final case class ApplyPOptional[S, T, A, B](s: S, optional: POptional[S, T, A, B]) {
  ...
  def some[A1, B1](implicit ev1: A =:= Option[A1], ev2: B =:= Option[B1]): ApplyPOptional[S, T, A1, B1] =
    adapt[Option[A1], Option[B1]].andThen(std.option.pSome[A1, B1])

  private[monocle] def adapt[A1, B1](implicit evA: A =:= A1, evB: B =:= B1): ApplyPOptional[S, T, A1, B1] =
    evB.substituteCo[ApplyPOptional[S, T, A1, *]](evA.substituteCo[ApplyPOptional[S, T, *, B]](this))

  def andThen[C, D](other: PSetter[A, B, C, D]): ApplyPSetter[S, T, C, D] =
    ApplyPSetter(s, optional.andThen(other))
  ...
  def andThen[C, D](other: PTraversal[A, B, C, D]): ApplyPTraversal[S, T, C, D] =
    ApplyPTraversal(s, optional.andThen(other))
  def andThen[C, D](other: POptional[A, B, C, D]): ApplyPOptional[S, T, C, D] =
    ApplyPOptional(s, optional.andThen(other))
  def andThen[C, D](other: PPrism[A, B, C, D]): ApplyPOptional[S, T, C, D] =
    ApplyPOptional(s, optional.andThen(other))
  def andThen[C, D](other: PLens[A, B, C, D]): ApplyPOptional[S, T, C, D] =
    ApplyPOptional(s, optional.andThen(other))
  def andThen[C, D](other: PIso[A, B, C, D]): ApplyPOptional[S, T, C, D] =
    ApplyPOptional(s, optional.andThen(other))
  ...
  @deprecated("use andThen", since = "3.0.0-M1")
  def composePrism[C, D](other: PPrism[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)
  @deprecated("use andThen", since = "3.0.0-M1")
  def composeLens[C, D](other: PLens[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)
  @deprecated("use andThen", since = "3.0.0-M1")
  def composeIso[C, D](other: PIso[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)
  ...
  @deprecated("use andThen", since = "3.0.0-M1")
  def ^<-?[C, D](other: PPrism[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)

  /** alias to composeLens */
  @deprecated("use andThen", since = "3.0.0-M1")
  def ^|->[C, D](other: PLens[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)

  /** alias to composeIso */
  @deprecated("use andThen", since = "3.0.0-M1")
  def ^<->[C, D](other: PIso[A, B, C, D]): ApplyPOptional[S, T, C, D] = andThen(other)
}

object ApplyPOptional {
  implicit def applyOptionalSyntax[S, A](self: ApplyOptional[S, A]): ApplyOptionalSyntax[S, A] =
    new ApplyOptionalSyntax(self)
}

/** Extension methods for monomorphic ApplyOptional */
final case class ApplyOptionalSyntax[S, A](private val self: ApplyOptional[S, A]) extends AnyVal {
  def each[C](implicit evEach: Each[A, C]): ApplyPTraversal[S, S, C, C] =
  ...
  def withDefault[A1](defaultValue: A1)(implicit evOpt: A =:= Option[A1]): ApplyOptional[S, A1] =
    self.adapt[Option[A1], Option[A1]].andThen(std.option.withDefault(defaultValue))

  def at[I, A1](i: I)(implicit evAt: At[A, i.type, A1]): ApplyOptional[S, A1] =
