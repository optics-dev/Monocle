...
final case class ApplyPLens[S, T, A, B](s: S, lens: PLens[S, T, A, B]) {
  ...
  private[monocle] def adapt[A1, B1](implicit evA: A =:= A1, evB: B =:= B1): ApplyPLens[S, T, A1, B1] =
    evB.substituteCo[ApplyPLens[S, T, A1, *]](evA.substituteCo[ApplyPLens[S, T, *, B]](this))

  def andThen[C, D](other: PSetter[A, B, C, D]): ApplyPSetter[S, T, C, D] =
    ApplyPSetter(s, lens.andThen(other))
  ...
  @deprecated("use andThen", since = "3.0.0-M1")
  def composeIso[C, D](other: PIso[A, B, C, D]): ApplyPLens[S, T, C, D] = andThen(other)
  ...
  /** alias to composeIso */
  @deprecated("use andThen", since = "3.0.0-M1")
  def ^<->[C, D](other: PIso[A, B, C, D]): ApplyPLens[S, T, C, D] = andThen(other)
}

object ApplyPLens {
  implicit def applyLensSyntax[S, A](self: ApplyLens[S, A]): ApplyLensSyntax[S, A] =
    new ApplyLensSyntax(self)
}

/** Extension methods for monomorphic ApplyLens */
final case class ApplyLensSyntax[S, A](private val self: ApplyLens[S, A]) extends AnyVal {
  ...
  def withDefault[A1](defaultValue: A1)(implicit evOpt: A =:= Option[A1]): ApplyLens[S, A1] =
