...
final class ApplyFoldOps[S](private val s: S) extends AnyVal {
  def applyFold[A](fold: Fold[S, A]): ApplyFold[S, A] = new ApplyFold[S, A](s, fold)
}

final class ApplyGetterOps[S](private val s: S) extends AnyVal{
  def applyGetter[A](getter: Getter[S, A]): ApplyGetter[S, A] = new ApplyGetter[S, A](s, getter)
}

final class ApplyIsoOps[S](private val s: S) extends AnyVal {
...
final class ApplyLensOps[S](private val s: S) extends AnyVal {
...
final class ApplyOptionalOps[S](private val s: S) extends AnyVal {
...
final class ApplyPrismOps[S](private val s: S) extends AnyVal {
...
final class ApplySetterOps[S](private val s: S) extends AnyVal {
  def applySetter[T, A, B](setter: PSetter[S, T, A, B]): ApplySetter[S, T, A, B] = new ApplySetter[S, T, A, B](s, setter)
}

final class ApplyTraversalOps[S](private val s: S) extends AnyVal {
