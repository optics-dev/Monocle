...
object PIso {
  ...
  /**
   * create a [[PIso]] between any type and itself. This [[PIso]] is the zero element of optics composition,
   * for all optics o of type O (e.g. Lens, Iso, Prism, ...):
   * o        composeIso Iso.zero == o
   * Iso.zero composeO   o        == o (replace composeO by composeLens, composeIso, composePrism, ...)
   */
  @inline def zero[S, T]: PIso[S, T, S, T] =
...
object Iso {
  ...
  /** alias for [[PIso]] zero when S = T and A = B */
  @inline def zero[S]: Iso[S, S] = PIso.zero[S, S]
