  ...
  implicit def oneAndIndex[T[_], A](implicit ev: Index[T[A], Int, A]): Index[OneAnd[T, A], Int, A] =
    new Index[OneAnd[T, A], Int, A]{
      def index(i: Int) = i match {
        case 0 => Optional[OneAnd[T, A], A](oneAnd => Maybe.just(oneAnd.head))((a, oneAnd) => oneAnd.copy(head = a))
        case _ => Optional[OneAnd[T, A], A](oneAnd => ev.index(i - 1).getMaybe(oneAnd.tail))(
